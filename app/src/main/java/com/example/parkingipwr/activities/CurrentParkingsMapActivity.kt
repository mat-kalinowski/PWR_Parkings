...
class CurrentParkingsMapActivity :  AppCompatActivity(), OnMapReadyCallback,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for(i in 0 until ParkingBase.getParkingsArray().size){
            val p = ParkingBase.getParkingsArray()[i]
    ...
    override fun onMapReady(googleMap: GoogleMap?) {
        ...
        val mark = LatLng(51.109353,17.058040)
        mMap = googleMap
    ...
    private fun addMarkers(){
        for(i in 0 until parkingPlaces.size){
            val p = parkingPlaces[i]
            val parkingInfo = ParkingBase.getParkingFromID(p.parking_id)
            if(parkingInfo != null){
    ...
    override fun onMarkerClick(clickedMarker: Marker?): Boolean {
        ...
        for(i in 0 until parkingMarkers.size){
            if(clickedMarker!! == parkingMarkers[i]){
                val parking = ParkingBase.getParkingFromID(parkingPlaces[i].parking_id)

                val dialog = Dialog(this)
                dialog.requestWindowFeature(Window.FEATURE_NO_TITLE)
                dialog.setContentView(R.layout.parking_info_dialog)
                (dialog.findViewById(R.id.textViewTitle) as TextView).text = parking!!.getNameString()
                (dialog.findViewById(R.id.textViewDescription) as TextView).text = parking.getDescriptionString()
                dialog.show()
            }
        }
        return true
